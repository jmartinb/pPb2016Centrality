...
void draw_compare_runs_1D(string var, const char* trigCut, const char* dataset, const char* cap, bool presclOn = true);
...
void compare_runs_1D()
{
  ...
  const char* dataset="pAMinimumBias1";
  const char* cap = "MBTrig_fullSel";
  
  TStopwatch timer;
  timer.Start();
  
  draw_compare_runs_1D("hiBin",selCut,dataset,cap);
  draw_compare_runs_1D("hiHF",selCut,dataset,cap);
  draw_compare_runs_1D("hiHFplus",selCut,dataset,cap);
  draw_compare_runs_1D("hiHFminus",selCut,dataset,cap);
  draw_compare_runs_1D("hiHFplusEta4",selCut,dataset,cap);
  draw_compare_runs_1D("hiHFminusEta4",selCut,dataset,cap);
  draw_compare_runs_1D("hiNpix",selCut,dataset,cap);
  draw_compare_runs_1D("hiNtracks",selCut,dataset,cap);
  draw_compare_runs_1D("hiNtracksPtCut",selCut,dataset,cap);
  draw_compare_runs_1D("hiNtracksEtaCut",selCut,dataset,cap);
  draw_compare_runs_1D("hiNtracksEtaPtCut",selCut,dataset,cap);
  draw_compare_runs_1D("sumPtVtx",selCut,dataset,cap);
  draw_compare_runs_1D("hiET",selCut,dataset,cap);;
  draw_compare_runs_1D("hiEE",selCut,dataset,cap);
  draw_compare_runs_1D("hiEB",selCut,dataset,cap);
  draw_compare_runs_1D("hiZDC",selCut,dataset,cap);
  draw_compare_runs_1D("hiZDCplus",selCut,dataset,cap);
  draw_compare_runs_1D("hiZDCminus",selCut,dataset,cap);
  draw_compare_runs_1D("zVtx",selCut,dataset,cap);//change many things !!
...
void draw_compare_runs_1D(string var, const char* trigCut, const char* dataset, const char* cap, bool presclOn){
  SetHistTitleStyle();
  SetyjPadStyle();
  gStyle->SetOptStat(0);
  TH1::SetDefaultSumw2();
  ...
  if(var == "hiNtracks" || var == "hiNtracksPtCut" || var == "hiNtracksEtaCut" || var == "hiNtracksEtaPtCut" || var == "sumPtVtx") xmax = 3500;
...
//    f1 = TFile::Open(Form("%sEventTree_PbPb_data_%s_run%s_15Feb.root",dir,dataset,run[i].data()));
  ...
  for(int j=0;j<Nset;j++){
    ...
    leg[j]->Draw();
    drawText(dataset,0.2,0.2+0.12);
    drawText(trigcap,0.2,0.2+0.06);
    drawText(evSelCut,0.2,0.2);
    can[j]->SaveAs(Form("runEvolution/compareBtwRuns_%s_run%s%s_%s_%s.pdf",dataset,runPrefix,run[Ncomp*j].data(),var.data(),cap));
  }
  cout << "c"<<endl;
  TCanvas* cratio[Nset];
  for(int j=0;j<Nset;j++){
    ...
    cratio[j]->SaveAs(Form("runEvolution/compareBtwRuns_%s_ratio_run%s%s_%s_%s.pdf",dataset,runPrefix,run[Ncomp*j].data(),var.data(),cap));
  }
}
